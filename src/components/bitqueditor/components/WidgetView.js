...
const WidgetView = observer(function WidgetView({children, widget, dataSource, ready}) {
                ...
                if ((dataSource && dataSource.historyDataSource) || (dataSource && dataSource.subscriptionDataSource)) {
                    ...
                    if (refChart.current.childNodes.length) {
                        ...
                        if (widget && (dataSource.historyDataSource || dataSource.subscriptionDataSource)) {
                            ...
                            window.ChartWidget = eval(`(${widget})`)
                            console.log('ChartWidget после eval:', window.ChartWidget);
                            const chartWidgetInstance = new window.ChartWidget(refChart.current, dataSource.historyDataSource, dataSource.subscriptionDataSource)
    ...
    return (
        <>
            ...
            <div ref={refJson}
                 className={"result-window result-window-json " + (currentQuery.widget_id !== 'config.widget' ? 'result-window-active' : '')}/>
            <div ref={refChart}
                 className={"result-window result-window-chart " + (currentQuery.widget_id === 'config.widget' ? 'result-window-active' : '')}/>
        </>
    )
})
